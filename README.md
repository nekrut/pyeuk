...
* **Live Interactive Demo**: An interactive demo report generated from the 153-specimen 2018 *Cyclospora* outbreak cohort is viewable at [`docs/example_report.html`](docs/example_report.html) (or online at [https://veg.github.io/pyeuk/example_report.html](https://veg.github.io/pyeuk/example_report.html)).
...
<p align="center">
  ...
  <br>
  <em>Figure: Standalone HTML report generated by <code>pyeuk report</code> on the 153-specimen 2018 <em>Cyclospora</em> outbreak cohort, showing confidence dendrogram, partition stability curve, transmission core roster (9 reproducible cores), and tree-ordered 153×153 wIBS distance heatmap (zero external JS/CDN dependencies).</em>
</p>

#### Interpreting the Outbreak Report Dashboard

The dashboard provides public health investigators and computational biologists with a multi-layered, mathematically grounded view of outbreak transmission dynamics without forcing arbitrary single-threshold partitions:

1. **Header Diagnostic Banner & Convergence Summary**
   * **Executive Summary**: Displays the supported cluster count interval and evidence consensus:
     > `4–7 clusters (count not determined; selectors scatter 2–12): solid splits give 4, adding moderate splits gives 7; 9 stable cores, 77% of specimen pairs resolved`
   * **Why "Count Not Determined"?**: Traditional typing algorithms arbitrarily force an integer cut $k$, papering over hierarchical outbreak structure. Here, the four independent unsupervised selectors (merge-height gap knee, mean silhouette, bootstrap partition stability, and gap statistic) scatter across $k \in [2, 12]$, reflecting nested epidemiological sub-lineages. PyEuk reports this uncertainty explicitly rather than forcing a fragile single number.
   * **Theme Switcher**: The button in the top-right corner toggles between dark and light themes dynamically without page reloads or network requests.

2. **Top-Level KPI Diagnostic Tiles**
   * **`153 specimens`**: The cohort size that passed completeness filtering ($\ge 10\%$ typed loci). All 153 clinical isolates were retained due to locus-dropout-tolerant pairwise calculations.
   * **`4–7 count`**: The empirical cluster count range supported by the multilocus genotype data.
   * **`no determined?`**: Indicates whether selectors converged on a single point estimate. "no" signals that the outbreak contains nested hierarchical structure.
   * **`9 stable cores`**: The number of robust transmission cores discovered ($\ge 90\%$ bootstrap co-assignment across resampled loci).
   * **`77% pairs resolved`**: The percentage of specimen pairs whose co-clustering probability across bootstrap replicates is definitive ($|2p_{ij} - 1| \ge 0.50$, i.e. $p_{ij} \ge 0.75$ or $p_{ij} \le 0.25$).
   * **`3 strong splits`**: Internal tree bifurcations reproduced in $\ge 75\%$ of bootstrap replicates.

3. **Panel A: Confidence Tree (Dendrogram)**
   * **Branch Style Encoding**:
     * **Thick solid black line (`confident split`)**: Branch split reproduced in $\ge 75\%$ of bootstrap resamplings. These mark indisputable epidemiological boundaries (e.g., distinguishing Vendor A restaurant isolates from Vendor B grocery store isolates).
     * **Thin gray line (`moderate`)**: Branches with moderate bootstrap support ($45\% \le \text{support} < 75\%$).
     * **Dashed gray line (`uncertain`)**: Internal sub-branching that varies across locus resamplings ($< 45\%$ support). This warns epidemiologists not to over-interpret micro-splits within a single distributor clade.
   * **Numbered Blue Leaf Bars**: Highlight specimen membership in the 9 transmission cores, corresponding directly to the Stable Cores roster table.

4. **Panel B: Count Sweep Profile (Silhouette & Stability Curve)**
   * **X-Axis ($k$)**: Evaluated cluster counts ($k \in [2, 10]$).
   * **Green Curve (Stability)**: Mean bootstrap partition stability across 100 resamplings.
   * **Blue Curve (Silhouette)**: Mean silhouette score across clusters.
   * **Shaded Vertical Band ($k \in [4, 7]$)**: The supported cluster count interval. When silhouette and stability rise together without a single distinct peak, it confirms the biological reality of hierarchical clustering rather than an artificial threshold.

5. **Panel C: Stable Cores Roster (Transmission Cores Table)**
   * **Epidemiological Interpretation**: Transmission cores represent connected components in the consensus graph where isolate pairs co-cluster in $\ge 90\%$ of bootstrap replicates ($p_{ij} \ge 0.90$).
   * **Core 1 (40 isolates)** & **Core 2 (31 isolates)**: Encompass the primary and secondary branches of the multistate pre-packaged salad outbreak (Vendor A).
   * **Core 3 (24 isolates)**: Encompasses the distinct vegetable tray outbreak (Vendor B).
   * **Peripheral Cores (Cores 4–9)**: Capture smaller state-specific sub-clusters and secondary retail exposures.
   * **Public Health Utility**: Cores define the indisputable epidemiological kernel of an outbreak, allowing investigators to trace confirmed transmission links without being misled by unstable peripheral isolates.

6. **Panel D: Distance Structure Heatmap**
   * **Tree-Ordered Symmetric Matrix ($153 \times 153$)**: Pairwise locus-dropout-tolerant weighted IBS (wIBS) genetic distances, reordered according to the hierarchical tree leaf order.
   * **Dark Diagonal Blocks**: Tightly linked isolates with low genetic distance ($d \approx 0$), directly corresponding to the 9 stable transmission cores.
   * **Off-Diagonal Structure**: Visualizes the clear genetic divergence between Vendor A and Vendor B outbreak strains, highlighting why binary traceback distribution chains are resolved with near-perfect concordance (ARI = 0.9737).
...
### Reproducing the 2018 *Cyclospora* Outbreak Investigation (153 Specimens)

The repository includes the complete 153-specimen 2018 *Cyclospora cayetanensis* foodborne outbreak dataset in [`example_data/cyclospora_outbreak/`](example_data/cyclospora_outbreak/). You can reproduce the distance evaluation, stability sweep, and visual report in three commands:

```bash
# 1. Compute pairwise locus-dropout-tolerant weighted IBS (wIBS) distances
pyeuk eukaryotyping \
    -i example_data/cyclospora_outbreak/haplotype_sheet_153.tsv \
    -o distance_matrix.tsv --wibs

# 2. Run hierarchical clustering with partition stability sweep (and inline report)
pyeuk cluster \
    -m distance_matrix.tsv \
    -o sweep/ \
    --k-min 2 --k-max 12 --n-boot 100 --report

# 3. Alternatively, render or customize the visual report separately
pyeuk report \
    sweep/*_SWEEP.json \
    --matrix distance_matrix.tsv \
    -o report.html \
    --flavor dashboard
```
Open `sweep/*_REPORT.html` (or `report.html`) in any browser to explore the interactive confidence dendrogram, partition sweep curves, transmission cores roster, and distance heatmap.

---

### End-to-End Amplicon Workflow (Raw Sequencing BAMs to Transmission Cores)

When starting from raw amplicon sequencing reads (Illumina paired-end):
